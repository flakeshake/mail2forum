...
class m2f_phpBB_insert_post
{
	...
	/** 
	* Constructor. Will only ever be run once, so we can happily load the phpBB files and let them 
	* 	initialise the necessary variables. In order for the variables to be available to the phpBB 
	* 	functions later, they must be declared global here.
	*  
	* @param string $phpbb_root_path full path to the phpBB installation
	* @return mixed true on success, or error object
	* @access public 
	*/
	function load($phpbb_root_path)
	{
		define('IN_PHPBB', TRUE);
		include($phpbb_root_path . 'extension.inc');
		
		if (m2f::is_error($error = $this->_check_db_connection($phpbb_root_path, $phpEx))) return $error;
	...
	* @access private 
	*/
	function _check_db_connection($phpbb_root_path, $phpEx)
	{
		if (!is_readable($phpbb_root_path . 'config.' . $phpEx)) return m2f::raise_error('Can\'t read phpBB config file at path "' . $phpbb_root_path . 'config.' . $phpEx . '".', __LINE__, __FILE__);
		include($phpbb_root_path . 'config.' . $phpEx);
		foreach (array('dbms', 'dbhost', 'dbuser', 'dbpasswd', 'dbname') as $var)
		{
		...
		if ($conn->connect($dbhost, $dbuser, $dbpasswd, $dbname) === FALSE) 
		{
			$error = ADODB_Pear_Error();
			return m2f::raise_error('Cannot connect to the database with settings in config.' . $phpEx . '. Error returned was: "' . $error->message . '".', __LINE__, __FILE__);
		}
		else
		{
			$conn->close();
	...
	* @access public 
	*/
	function insert($message, $subject, $forum_id, $user_id, $username, $user_attach_sig = TRUE, $topic_id = NULL, $current_time = NULL)
	{
		...
		if ( !$this->db->sql_query($sql, END_TRANSACTION) )
		{
			return m2f::raise_error('Bad SQL statement in phpBB insert_post function: "' . $sql . '".', __LINE__, __FILE__);
		}
